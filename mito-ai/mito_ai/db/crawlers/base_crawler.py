...
def crawl_db(
    ...
    try:
        merged_connect_args: Dict[str, Any] = dict(connect_args or {})
        if db_type == "mssql":
            # For Microsoft SQL Server, we need to trust the server certificate
            merged_connect_args.setdefault("TrustServerCertificate", "yes")

        if merged_connect_args:
            engine = create_engine(conn_str, connect_args=merged_connect_args)
        else:
        ...
        with engine.connect() as connection:
            ...
            result = connection.execute(text(tables_query), {"schema": schema_name})
