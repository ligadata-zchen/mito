...
class OpenAIClient:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # When set, these overrides force a specific OpenAI-compatible endpoint
        # (e.g. Ollama) and bypass the env-var-driven detection in _build_openai_client
        # and the global model overrides in _adjust_model_for_provider.
    ...
    def capabilities(self) -> AICapabilities:
        ...
        if constants.OLLAMA_MODEL and not constants.OLLAMA_MODELS:
            return AICapabilities(
                configuration={
    ...
    def _build_openai_client(self) -> Optional[Union[openai.AsyncOpenAI, openai.AsyncAzureOpenAI]]:
        ...
        # Constructor overrides take precedence (used by the Ollama router arm).
        if self._base_url_override is not None and self._api_key_override is not None:
        ...
        elif constants.OLLAMA_MODEL and not constants.OLLAMA_MODELS:
            # Legacy single-model Ollama. Multi-model mode is handled per-call by
            # the router arm in provider_manager, which constructs OpenAIClient
    ...
    def _adjust_model_for_provider(self, model: str) -> str:
        ...
        # Ollama router mode (this client was constructed with explicit overrides):
        # the model arrives prefixed as "ollama/<name>" and must be stripped before
        # being sent to Ollama, which doesn't recognize the prefix.
        if self._base_url_override is not None and model.lower().startswith('ollama/'):
            stripped_model = strip_router_prefix(model)
            self.log.debug(f"Stripping Ollama prefix: {model} -> {stripped_model}")
        ...
        # Legacy single-model Ollama: any picked model gets force-overridden to OLLAMA_MODEL.
        # Skipped when the multi-model router (OLLAMA_MODELS) is configured, so the
        # picked ollama/<name> wins instead.
        if constants.OLLAMA_MODEL is not None and not constants.OLLAMA_MODELS:
