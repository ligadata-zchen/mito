...
def get_open_ai_completion_function_params(
    ...
    *,
    ...
    if response_format_info:
        ...
        # Ollama-served open-source models (Qwen, Kimi, DeepSeek, etc.) likewise have
        # inconsistent strict-JSON-schema support across model families, so we use the
        # same lenient format for them.
        if (
            not force_full_json_schema_response_format
            and (
                is_abacus_configured()
                or model.lower().startswith('abacus/')
                or model.lower().startswith('ollama/')
            )
        ):
