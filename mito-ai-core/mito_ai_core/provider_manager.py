...
class ProviderManager:
    ...
    def key_type(self) -> str:
        ...
        if (
            ...
            or constants.OLLAMA_MODEL
            or constants.OLLAMA_MODELS
        ):
            return USER_KEY
        ...
        for attempt in range(max_retries + 1):
            try:
                if model_type == "copilot":
                    ...
                    # Ollama exposes an OpenAI-compatible /v1 endpoint, so we reuse OpenAIClient
                    # with the Ollama base URL. The `ollama/` prefix is stripped inside
                    # OpenAIClient._adjust_model_for_provider before the API call.
                    if not constants.OLLAMA_BASE_URL:
                    ...
                    completion = await ollama_client.request_completions(
                        message_type=message_type,
                        messages=messages,
                        model=resolved_model,
                        response_format_info=response_format_info,
    ...
    async def stream_completions(
        ...
        try:
            if model_type == "copilot":
                ...
                accumulated_response = await ollama_client.stream_completions(
                    message_type=message_type,
                    messages=messages,
                    model=resolved_model,
                    message_id=message_id,
            ...
            mlobs.record_completion(
                ...
                model=resolved_model,
                message_type=message_type.value,
                completion=accumulated_response,
                input_tokens=estimated_input_tokens,
                duration_ms=total_response_time_ms,
                success=True,
                streaming=True,
                ttft_ms=ttft_ms,
                thread_id=thread_id,
            ...
            mlobs.record_completion(
                ...
                model=resolved_model,
                message_type=message_type.value,
                completion=accumulated_response if accumulated_response else None,
                duration_ms=int((time.monotonic() - request_start) * 1000),
                success=False,
                error=e,
                streaming=True,
