...
def get_available_models() -> List[str]:
    ...
    # Check if enterprise mode is enabled AND LiteLLM is configured
        ...
        # Return standard models, optionally extended with Ollama-served open-source models.
        models = list(STANDARD_MODELS)
        if constants.OLLAMA_MODELS:
            models.extend(f"ollama/{m}" for m in constants.OLLAMA_MODELS)
        return models
...
def get_fast_model_for_selected_model(selected_model: str) -> str:
    ...
    """
    # Ollama-served models are user-supplied with no canonical fast/smart ordering.
