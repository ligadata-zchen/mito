...
# Ollama
# OLLAMA_MODEL (singular): legacy single-model mode — every request is forced to this model.
# OLLAMA_MODELS (plural, comma-separated): multi-model router mode — each entry is exposed
# as `ollama/<name>` in the model picker and routed through the OpenAI-compatible endpoint.
...
# Ollama multi-model config (open-source models served via Ollama's OpenAI-compatible /v1 API)
OLLAMA_MODELS_STR = os.environ.get("OLLAMA_MODELS", "")
OLLAMA_MODELS = parse_comma_separated_models(OLLAMA_MODELS_STR)
